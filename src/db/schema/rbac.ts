import { boolean, index, integer, pgTable, primaryKey, text, timestamp, uniqueIndex, uuid, varchar } from "drizzle-orm/pg-core";
...
export const mfaFactors = pgTable("mfa_factors", { id:id(), userId:uuid("user_id").references(()=>users.id,{onDelete:"cascade"}).notNull(), type:varchar("type",{length:30}).notNull(), secretCiphertext:text("secret_ciphertext").notNull(), label:varchar("label",{length:100}), lastUsedStep:integer("last_used_step"), verifiedAt:timestamp("verified_at",{withTimezone:true}), disabledAt:timestamp("disabled_at",{withTimezone:true}), createdAt:createdAt() },t=>[index("mfa_factors_user_idx").on(t.userId)]);

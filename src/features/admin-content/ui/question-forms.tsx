...
export function QuestionForm({ topics, question, sectionId, onAdded, onCancel, returnTo }: { topics: TopicOption[]; question?: EditableQuestion; sectionId?: string; onAdded?: () => void; onCancel?: () => void; returnTo?: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (type === "SINGLE_CHOICE" && correctCount !== 1) { setError("Select exactly one correct answer before saving."); return; }
    if (type === "MULTIPLE_CHOICE" && correctCount < 2) { setError("Select at least two correct answers before saving."); return; }
    setBusy(true);

...
export function parseQuestionCsv(csv: string, selectedTopicId?: string): { questions: QuestionInput[]; issues: ImportIssue[]; totalRows: number } {
  ...
  const headers = rows[0].map((header) => header.trim()); const requiredHeaders = selectedTopicId ? ["stem"] : QUESTION_CSV_HEADERS; const missing = requiredHeaders.filter((header) => !headers.includes(header));
  ...
  rows.slice(1).forEach((values, offset) => {
    ...
    const candidate = { topicId: selectedTopicId ?? record.topicId, type: record.type || (selectedTopicId ? "SINGLE_CHOICE" : undefined), stem: record.stem, explanation: record.explanation, marks: Number(record.marks || (selectedTopicId ? 1 : 0)), negativeMarks: Number(record.negativeMarks || 0), difficulty: record.difficulty || (selectedTopicId ? "MEDIUM" : undefined), options: optionKeys.flatMap((key, sortOrder) => record[`option${key}`] ? [{ stableKey: key, body: record[`option${key}`], isCorrect: correct.has(key), sortOrder }] : []), numericAnswer: record.numericAnswer ? Number(record.numericAnswer) : null, numericTolerance: record.numericTolerance ? Number(record.numericTolerance) : 0, acceptedAnswers: (record.acceptedAnswers ?? "").split("|").map((answer) => answer.trim()).filter(Boolean), caseSensitive: truthy(record.caseSensitive ?? "false") };
...
// Test imports use the topic chosen in the builder, never a hidden spreadsheet UUID.
export function testQuestionCsvTemplate() {
  return 'stem,optionA,optionB,optionC,optionD,correctOptions,explanation\n"What is 2 + 2?",3,4,5,6,B,"Two plus two equals four."';
}

...
export async function Overview({ adminOnly = false }: { adminOnly?: boolean }) {
  ...
  return <WorkspaceShell admin={auth.admin} name={auth.user.name} permissions={auth.permissions} section="overview">
    <div className="page-heading"><span className="eyebrow">{auth.admin ? "YOUR CONTROL CENTRE" : "MAKE ROOM FOR PROGRESS"}</span><h1>{auth.admin ? "Your admin workspace" : "Your preparation starts here"}</h1><p>Welcome, {auth.user.name}. {auth.admin ? "Manage content through permission-checked workflows." : "Your account is ready. Take a moment to make it yours."}</p></div>
    ...
    <section className="panel empty-state"><div className="empty-symbol" aria-hidden="true">▤</div><span className="eyebrow">{canManageExams ? "PHASE 5" : "COMING NEXT"}</span><h2>{canManageExams ? "Content management is ready" : auth.admin ? "Your assigned tools will appear here" : "Your practice space is getting ready"}</h2><p>{canManageExams ? "Build the exam taxonomy now. Question review, tests and imports will be added in the next Phase 5 slices." : auth.admin ? "Navigation is generated from your server-side permissions." : "Mock tests and results will appear here when the practice modules are available."}</p>{canManageExams ? <Link className="button secondary" href="/admin/exams">Manage exams</Link> : <span className="quiet-tag">No action needed right now</span>}</section>
  </WorkspaceShell>;
}

...
type TestDetails = { category: TestCategory | null; id: string; examId: string; title: string; mode: "PRACTICE" | "MOCK" | "LIVE"; durationMinutes: number; instructions: string | null; maxAttempts: number; shuffleQuestions: boolean; shuffleOptions: boolean; status: string };
...
function payload(data: FormData) { return { examId: String(data.get("examId")), title: String(data.get("title")), mode: "MOCK", category: data.get("category") || null, durationMinutes: Number(data.get("durationMinutes")), instructions: String(data.get("instructions") ?? ""), maxAttempts: Number(data.get("maxAttempts")), shuffleQuestions: data.get("shuffleQuestions") === "on", shuffleOptions: data.get("shuffleOptions") === "on" }; }

function CategoryField({ value = "FULL_MOCK" }: { value?: TestCategory | null }) {
  return <label className="field"><span>Test type</span><select name="category" defaultValue={value ?? ""} required>
    {!value && <option value="" disabled>Choose a test type</option>}
    {Object.entries(testCategoryLabels).map(([key, label]) => <option key={key} value={key}>{label}</option>)}
  </select><small>Subject test: questions from one subject. Topic set: questions from one topic. Full mock: combine subjects as needed.</small></label>;
}

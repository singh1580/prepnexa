...
export type MaterialInput = { examId: string; title: string; type: "ARTICLE" | "PDF" | "VIDEO" | "FILE"; body: string; privateObjectKey: string; allowDownload?: boolean };
export type StoredFileMetadata = { objectKey: string; originalFileName: string; contentType: string; checksum: string; sizeBytes: number };
...
export function listMaterials() { return db.select({ id: materials.id, title: materials.title, type: materials.type, status: materials.status, examName: exams.name, allowDownload: materials.allowDownload }).from(materials).innerJoin(exams, eq(materials.examId, exams.id)).orderBy(desc(materials.updatedAt)); }
...
export async function insertMaterial(input: MaterialInput, audit: Audit, file?: StoredFileMetadata) { const id = randomUUID(); const versionId = randomUUID(); const now = new Date(); const objectKey = (file?.objectKey ?? input.privateObjectKey) || null; const allowDownload = input.allowDownload ?? false; const values = { id, examId: input.examId, title: input.title, type: input.type, body: input.body || null, privateObjectKey: objectKey, allowDownload, status: "DRAFT" as const, createdBy: audit.actorUserId, createdAt: now, updatedAt: now }; await db.batch([db.insert(materials).values(values), db.insert(materialVersions).values({ id: versionId, materialId: id, version: 1, title: input.title, body: input.body || null, privateObjectKey: objectKey, originalFileName: file?.originalFileName, contentType: file?.contentType, checksum: file?.checksum, sizeBytes: file?.sizeBytes, createdBy: audit.actorUserId, createdAt: now }), db.insert(auditLogs).values({ actorUserId: audit.actorUserId, action: "material.created", entityType: "material", entityId: id, requestId: audit.requestId, after: { examId: input.examId, title: input.title, type: input.type, allowDownload, status: "DRAFT", version: 1, file: file ? { originalFileName: file.originalFileName, contentType: file.contentType, checksum: file.checksum, sizeBytes: file.sizeBytes } : undefined } })]); return { id }; }
export async function findLatestMaterialVersion(materialId: string) { return db.query.materialVersions.findFirst({ where: eq(materialVersions.materialId, materialId), orderBy: desc(materialVersions.version) }); }
...
export async function patchMaterial(before: NonNullable<Awaited<ReturnType<typeof findMaterial>>>, input: MaterialInput, audit: Audit) {
  ...
  await db.batch([
    db.update(materials).set({ examId: input.examId, title: input.title, type: input.type, body: input.body || null, privateObjectKey: input.privateObjectKey || before.privateObjectKey, allowDownload: input.allowDownload ?? false, updatedAt: new Date() }).where(and(eq(materials.id, before.id), eq(materials.status, "DRAFT"))),
    db.insert(materialVersions).values({ materialId: before.id, version, title: input.title, body: input.body || null, privateObjectKey: input.privateObjectKey || before.privateObjectKey, createdBy: audit.actorUserId }),
...
export async function appendMaterialFileVersion(before: NonNullable<Awaited<ReturnType<typeof findMaterial>>>, file: StoredFileMetadata, input: { title: string; allowDownload: boolean }, audit: Audit) {
  const [latest] = await db.select({ version: materialVersions.version }).from(materialVersions).where(eq(materialVersions.materialId, before.id)).orderBy(desc(materialVersions.version)).limit(1);
  const version = (latest?.version ?? 0) + 1;
  await db.batch([
    db.update(materials).set({ title: input.title, type: file.contentType === "application/pdf" ? "PDF" : "FILE", privateObjectKey: file.objectKey, allowDownload: input.allowDownload, updatedAt: new Date() }).where(and(eq(materials.id, before.id), eq(materials.status, "DRAFT"))),
    db.insert(materialVersions).values({ materialId: before.id, version, title: input.title, privateObjectKey: file.objectKey, originalFileName: file.originalFileName, contentType: file.contentType, checksum: file.checksum, sizeBytes: file.sizeBytes, createdBy: audit.actorUserId }),
    db.insert(auditLogs).values({ actorUserId: audit.actorUserId, action: "material.file_uploaded", entityType: "material", entityId: before.id, requestId: audit.requestId, after: { version, originalFileName: file.originalFileName, contentType: file.contentType, checksum: file.checksum, sizeBytes: file.sizeBytes } }),
  ]);

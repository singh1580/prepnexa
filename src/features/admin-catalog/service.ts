...
import { copyProductRecord, patchProduct, patchMaterial, unlinkProductRecord, archiveCatalogRecord, materialHasPublishedPackage, findCatalogTest, findMaterial, findProduct, findProductBundle, insertMaterial, insertProduct, insertProductLink, listCatalogExams, listMaterials, listProducts, publishMaterialRecord, publishProductRecord, type MaterialInput, type ProductInput } from "./repository";
...
export const createProduct = (input: ProductInput, actor: Actor) => write("product_create", actor, () => insertProduct(input, { actorUserId: actor.userId, requestId: actor.requestId }));
export async function createMaterial(input: MaterialInput, actor: Actor) { if (!await findExam(input.examId)) throw contentNotFound("Exam"); return write("material_create", actor, () => insertMaterial(input, { actorUserId: actor.userId, requestId: actor.requestId })); }
...
export async function getMaterial(id: string) { const value = await findMaterial(id); if (!value) throw contentNotFound("Material"); return value; }
...
export async function duplicateMaterial(id: string, actor: Actor) {
  const before = await getMaterial(id);
  return createMaterial({ examId: before.examId, title: before.title.slice(0, 190) + " (copy)", type: before.type, body: before.body ?? "", privateObjectKey: before.privateObjectKey ?? "" }, actor);
}

...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <WorkspaceShell admin name={auth.user.name} permissions={auth.permissions} section="exams">
    ...
    <header className="page-heading split-heading"><div><span className="eyebrow">EXAM STRUCTURE</span><h1>{exam.name}</h1><p>{draft ? "Organise subjects and topics before publishing." : "Published structure is locked to protect linked content."}</p></div><span className={`status-pill content-${exam.status.toLowerCase()}`}>{exam.status.replace("_", " ")}</span></header>

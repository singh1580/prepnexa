...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  const topics = test.status === "DRAFT" ? await listTopicsForTest(test.examId) : [];
  const assigned = new Set(test.sections.flatMap((section) => section.questions.map((question) => question.questionId)));
  return <WorkspaceShell admin name={auth.user.name} permissions={auth.permissions} section="tests"><Link className="back-link" href="/admin/tests">← All tests</Link><header className="page-heading split-heading"><div><span className="eyebrow">{test.examName} · {testCategoryLabel(test.category)}</span><h1>{test.title}</h1><p>{test.durationMinutes} minutes · {test.maxAttempts} maximum attempt{test.maxAttempts === 1 ? "" : "s"}</p></div><span className={`status-pill content-${test.status.toLowerCase()}`}>{test.status}</span></header><TestBuilder test={{ category: test.category, id: test.id, examId: test.examId, title: test.title, mode: test.mode, durationMinutes: test.durationMinutes, instructions: test.instructions, maxAttempts: test.maxAttempts, shuffleQuestions: test.shuffleQuestions, shuffleOptions: test.shuffleOptions, status: test.status }} exams={exams} sections={test.sections.map((section) => ({ id: section.id, title: section.title, durationMinutes: section.durationMinutes, sortOrder: section.sortOrder, questions: section.questions.map((question) => ({ questionId: question.questionId, stem: question.stem, type: question.type, sortOrder: question.sortOrder, explanation: question.explanation, marks: question.marks, negativeMarks: question.negativeMarks, options: question.options, answerConfig: question.answerConfig })) }))} questions={test.availableQuestions.filter((question) => !assigned.has(question.id))} canManage={canManage} canCreate={auth.permissions.includes(CONTENT_PERMISSIONS.createQuestions)} canPublish={auth.permissions.includes(CONTENT_PERMISSIONS.publishQuestions)} topics={topics} /></WorkspaceShell>;
}

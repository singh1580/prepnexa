...
export default async function Page() {
  ...
  return <WorkspaceShell admin name={auth.user.name} permissions={auth.permissions} section="questions">
    <header className="page-heading split-heading"><div><span className="eyebrow">QUESTION BANK</span><h1>Questions with a paper trail</h1><p>Author, review and publish immutable revisions through separate permissions.</p></div>{canCreate && <Link className="button" href="/admin/questions/new">Create question</Link>}</header>
    <div className="question-summary"><span><strong>{questions.length}</strong> total</span><span><strong>{questions.filter((question) => question.status === "DRAFT").length}</strong> drafts</span><span><strong>{questions.filter((question) => question.status === "IN_REVIEW").length}</strong> in review</span><span><strong>{questions.filter((question) => question.status === "PUBLISHED").length}</strong> published</span></div>

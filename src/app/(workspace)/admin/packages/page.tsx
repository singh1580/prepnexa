...
export default async function Page() {
  ...
  return <WorkspaceShell admin name={auth.user.name} permissions={auth.permissions} section="packages">
    <header className="page-heading"><span className="eyebrow">CATALOG OPERATIONS</span><h1>Package access, provider-neutral</h1><p>Define sellable packages and learning-material metadata. Payment providers and protected delivery remain separate later phases.</p></header>
    <div className="catalog-admin-grid">
      {canProducts ? <section><div className="section-heading"><div><span className="eyebrow">PACKAGES</span><h2>Access bundles</h2></div></div>{products.length ? <div className="managed-list">{products.map((product) => <Link className="managed-card" href={`/admin/packages/${product.id}`} key={product.id}><div><span className="eyebrow">₹{(product.pricePaise / 100).toLocaleString("en-IN")} · {product.accessDays} DAYS</span><h2>{product.name}</h2><p>{product.testCount} tests · {product.materialCount} materials</p></div><span className={`status-pill content-${product.status.toLowerCase()}`}>{product.status}</span><b>→</b></Link>)}</div> : <div className="panel empty-state compact-empty"><h2>No packages yet</h2></div>}<section className="panel"><span className="eyebrow">NEW PACKAGE</span><h2>Create metadata</h2><ProductCreateForm /></section></section> : null}
      {canMaterials ? <section><div className="section-heading"><div><span className="eyebrow">MATERIALS</span><h2>Versioned metadata</h2></div></div><div className="material-list">{materials.map((material) => <article className="panel" key={material.id}><span className="eyebrow">{material.examName} · {material.type}</span><h2>{material.title}</h2><div className="material-action"><span className={`status-pill content-${material.status.toLowerCase()}`}>{material.status}</span>{material.status === "DRAFT" ? <MaterialPublishButton id={material.id} /> : null}</div></article>)}</div>{exams.length > 0 ? <section className="panel"><span className="eyebrow">NEW MATERIAL</span><h2>Create version 1</h2><MaterialCreateForm exams={exams} /></section> : null}</section> : null}
    </div>
  </WorkspaceShell>;
}

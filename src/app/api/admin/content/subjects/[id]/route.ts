...
import { updateSubject } from "@/features/admin-content/service";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return executeRoute(request, async (requestId) => {
    ...
    return successResponse(await updateSubject(id, subjectInputSchema.parse(await request.json()), { userId: auth.user.id, requestId }), requestId);
  });
}

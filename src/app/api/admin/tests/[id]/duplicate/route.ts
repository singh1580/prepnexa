...
import { duplicateTest } from "@/features/admin-tests/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  return executeRoute(request, async (requestId) => {
    ...
    return successResponse(await duplicateTest(testIdSchema.parse((await params).id), { userId: auth.user.id, requestId }), requestId);
  });
}

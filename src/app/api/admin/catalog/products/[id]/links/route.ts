...
import { linkProduct, unlinkProduct } from "@/features/admin-catalog/service";
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) { return executeRoute(request, async (requestId) => { const auth = await requirePermission(CONTENT_PERMISSIONS.manageProducts); const input = productLinkSchema.parse(await request.json()); const result = await unlinkProduct(catalogIdSchema.parse((await params).id), input.kind, input.id, { userId: auth.user.id, requestId }); return successResponse(result, requestId, { status: 201 }); }); }
